...
struct {
    ...
    __type(value, __u32); // peer index in sock_map
} cookie_to_peer_index SEC(".maps");

// Global stats using per-CPU array for high performance
struct {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 2); // index 0: total_bytes, index 1: total_packets
    __type(key, __u32);
    __type(value, __u64);
} stats SEC(".maps");
